...
public class I_ProductServiceImpl implements I_ProductService {
    ...
    public ResponseDto<ProductRequest.DetailResponse> create(
            UserPrincipal userPrincipal,
            ProductRequest.@Valid Create req
    ) {
        ProductRequest.DetailResponse data = null;
        ...
        data = new ProductRequest.DetailResponse(saved.getId(), saved.getName(), saved.getPrice());
    ...
    public ResponseDto<ProductResponse.DetailResponse> update(
        ...
        return ResponseDto.setSuccess("제품이 성공적으로 등록 되었습니다.", data );
    }

    @Override
    public ResponseDto<ProductRequest.DetailResponse> get(Long productId) {
        ProductRequest.DetailResponse data = null;

        I_Product product = productRepository.findById(productId)
                .orElseThrow(() -> new EntityNotFoundException("상품을 찾을 수 없습니다"));

        data = new ProductRequest.DetailResponse(product.getId(), product.getName(), product.getPrice());
