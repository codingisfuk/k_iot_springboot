...
/*
    ...
    : 스프링을 쉽고 빠르게 사용할 수 있도록 지원하는 프레임위크 확장판
    ...
    2) DS >> 핸들러 매핑
        : 해당 요청(URL, HTTP 매서드 등)에 대응하는 핸들러(= 컨트롤러 메서드)를 찾음
        - 핸들러 매핑은 적합한 컨트롤러/핸들러 점보를 찾아서 반환

    3) DS >> 핸들러 어댑터
        : DS가 선택된 핸들러를 실행할수 있는 어댑터를 찾아 핸들러 어댑터를 호출
        - 해당 어댑터는 실제 컨트롤러와 연결되어 컨트롤러의 메서드를 실행하고
                , 실행 결과(모델 & 뷰 이름 들)를 DS에 반환
        >> 어댑터
            : 스프링은 컨트롤러가 스타일이 달라고 공통 방식으로 호출 할 수 있도록 어댑터가 중간에서 표준화
            (@Controller, @RestController 등)

    4) 핸들러 어댑터 >> 컨트롤러
        : 컨트롤러가 요청 파라미터를 받아 비즈니스 로직 호출, 모델 데이터 생성, 뷰 이름(String) 등을 반환
        - @RestController 인 경우 뷰를 반환하지 않고 바디(JSON 등)로 바로 전달
            >> 뷰/ 뷰 리졸버 단계가 생략 가능
...
/*
    ...
    1. 서블릿: 각 서블릿이 컨트롤러 역할까지 직접 함 - 구조를 스스로 만들고, 규칙도 스스로 정해야 함
    ...
    2. 스프링부트(MVC)
        ...
        2) 서블릿 레벨이 DispatcherServlet 1개면 충분 (스프링이 자동 등록)
            - 컨트롤러 클래스/메서드는 여러 개
                >> 애플리케이션 코드 레벨
                 " 서블릿 여러 개"가 아니라 POO(Plain Old Java Object) 단위라 가볍고 일관적
