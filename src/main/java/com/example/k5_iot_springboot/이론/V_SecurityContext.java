...
/*
    ...
    2. AuthenticateManager.authenticate(...) 호출
        - 구현체는 ProviderManager
        - 내부에서 여러 AuthenticationProvider 리스트가 있고
            , 토큰 타입을 처리할 수 있는 Provider 순서대로 위임
    ...
    6. 비밀번호 검증 (PasswordEncoder)
        - PasswordEncoder.natches(raw, encoded)
    ...
    8. 인증 성공 토큰 생성
        ...
        - 상태L authenticated=true, principal=UserPrincipal, credentials=null(보안상 삭제), authorities 포함
